...
def BiFpnLayer(P2, P3, P4, P5, filters=64, conv_input=True, has_bottomup=True, name=None):
    ...
    if conv_input:
        P2_in = conv2d(P2_in, filters=filters, stride=1, n=1,
                       kernel_size=1, name="P2_in_conv_channels")
        P3_in = conv2d(P3_in, filters=filters, stride=1, n=1,
                       kernel_size=1, name="P3_in_conv_channels")
        P4_in = conv2d(P4_in, filters=filters, stride=1, n=1,
                       kernel_size=1, name="P4_in_conv_channels")
        P5_in = conv2d(P5_in, filters=filters, stride=1, n=1,
                       kernel_size=1, name="P5_in_conv_channels")

    # compute P4_td
    P5_up = UpSampling2D(name="P5_in_up")(P5_in)
    P4_td = FastNormalizedFusion(name="P4_td_add")([P4_in, P5_up])
    P4_td = conv2d(P4_td, filters=filters, stride=1,
                   n=1, kernel_size=3, name="P4_td_conv")

    # compute P3_td
    P4_td_up = UpSampling2D(name="P4_td_up")(P4_td)
    P3_td = FastNormalizedFusion(name="P3_td_add")([P3_in, P4_td_up])
    P3_td = conv2d(P3_td, filters=filters, stride=1,
                   n=1, kernel_size=3, name="P3_td_conv")

    # compute P2_out
    P3_td_up = UpSampling2D(name="P3_td_up")(P3_td)
    ...
    P3_out = FastNormalizedFusion(name="P3_out_add")(
        [P3_in, P3_td, P2_out_down])
    P3_out = conv2d(P3_out, filters=filters, stride=1,
                    n=1, kernel_size=3, name="P3_out_conv")

    # compute P4_out
    P3_out_down = MaxPooling2D(padding="same", name="P3_out_down")(P3_out)
    P4_out = FastNormalizedFusion(name="P4_out_add")(
        [P4_in, P4_td, P3_out_down])
    P4_out = conv2d(P4_out, filters=filters, stride=1,
                    n=1, kernel_size=3, name="P4_out_conv")

    # compute P5_out
    P4_out_down = MaxPooling2D(padding="same", name="P4_out_down")(P4_out)
    P5_out = FastNormalizedFusion(name="P5_out_add")([P4_out_down, P5_in])
    P5_out = conv2d(P5_out, filters=filters, stride=1,
                    n=1, kernel_size=3, name="P5_out_conv")
