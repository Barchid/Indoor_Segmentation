...
class DepthGradCam:
    def __init__(self, model, depth_range=(0, 25), layer_name=None):
        self.model = model
    ...
    def compute_heatmap(self, image, eps=1e-8):
        ...
        with tf.GradientTape() as tape:
            ...
            greater_equal = tf.greater_equal(
                predictions, self.depth_range[0]/255.)

            lower_equal = tf.less_equal(predictions, self.depth_range[1]/255.)

            logic_and = tf.cast(
                tf.logical_and(greater_equal, lower_equal),
                "float32"
            )

            filtered_depth = predictions * logic_and

            loss = tf.reduce_sum(filtered_depth)
